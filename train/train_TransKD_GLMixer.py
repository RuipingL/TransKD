...
from CSF_GLMixer import build_kd_trans, hcl, CriterionCWD
...
def train(args, model, teacher):
    ...
    if args.dataset == 'cityscapes':
                savedir = f'../save/Testbatch{args.batch_size}/Optimization_PEA/{args.knowledge_distillation_feature_fusion}-{args.knowledge_distillation_loss}{embedd}-AttentionGLU4-{args.embed_weights}'
    elif args.dataset == 'ACDC':
        savedir = f'../save/Testbatch{args.batch_size}-ACDC/Optimization_PEA/{args.knowledge_distillation_feature_fusion}-{args.knowledge_distillation_loss}{embedd}-AttentionGLU4-{args.embed_weights}'
    else:
        assert 'Dataset does not exist'
    if args.add:
        savedir = savedir +'-add'
...
def main(args):
    ...
    model = build_kd_trans(model, args.embed, args.add)
