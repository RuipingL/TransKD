...
from models.Segformer4EmbeddingKD import mit_b2#,mit_b3,mit_b4,mit_b5
...
def train(args, model, teacher):
    ...
    if args.dataset == 'cityscapes':
        savedir = f'../save_lvt/Testbatch{args.batch_size}/Optimization/TransKDBase'
    elif args.dataset == 'ACDC':
        savedir = f'../save_lvt/Testbatch{args.batch_size}-ACDC/Optimization/TransKDBase'
    else:
        assert 'Dataset does not exist'
    ...
    if args.savedate:
        savefile = f'{args.model}-TransKDBase-{pretrained}-{TODAY}'
    else:    
        savefile = f'{args.model}-TransKDBase-{pretrained}'
...
def main(args):
    ...
    model = build_kd_trans(model, args.embed,[64,64,160,256])
