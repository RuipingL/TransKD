...
from models.Segformer import mit_b2,mit_b3,mit_b4,mit_b5
...
class MyCoTransform(object):
    def __init__(self, augment=True, height=512, model='SegformerB2'):
...
def main(args):
    if args.dataset == 'cityscapes':
        savedir = f'../save/teachers'
        if args.height != 512:
            savedir = f'../save-{args.height}/teachers'

    elif args.dataset == 'ACDC':
        savedir = '../save/ACDC_teachers'
    else:
        assert 'Dataset does not exist'
    ...
    if args.model.startswith('Segformer'):
        if args.model.endswith('B2'):
            model = mit_b2()
            path = 'ckpt_pretrained/mit_b2.pth'
        elif args.model.endswith('B3'):
            model = mit_b3()
            path = 'ckpt_pretrained/mit_b3.pth'        
        elif args.model.endswith('B4'):
            model = mit_b4()
            path = 'ckpt_pretrained/mit_b4.pth'              
        elif args.model.endswith('B5'):
            model = mit_b5()
            path = 'ckpt_pretrained/mit_b5.pth'               
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="SegformerB2", help ='Teacher for Knowledge Distillation. Default: SegformerB2')

    parser.add_argument('--port', type=int, default=8097)
    parser.add_argument('--datadir', default="/cvhci/temp/rliu/Dataset/cityscapes/")
    ...
    parser.add_argument('--savedir', default = 'teachers')
