...
from CSF import build_kd_trans, hcl, CriterionCWD
...
def train(args, model, teacher):
    ...
    if args.dataset == 'cityscapes':
        savedir = f'../save_pvtv2/Testbatch{args.batch_size}/Optimization/TransKDBase'
    elif args.dataset == 'ACDC':
        savedir = f'../save_pvtv2/Testbatch{args.batch_size}-ACDC/Optimization/TransKDBase'
    else:
        assert 'Dataset does not exist'
...
def load_my_state_dict(model, state_dict, multidevice):  
...
def main(args):
    ...
    teacher = build_pvts("pvt_v2_b2", NUM_CLASSES)
    if args.dataset == 'cityscapes':
        teacher_path = '../outputs/pvtv2b2_teacher_cityscapes.th'
    else:
        assert 'dataset not supported'


    model = build_pvts("pvt_v2_b0", NUM_CLASSES)
    path = 'ckpt_pretrained/pvt_v2_b0.pth'
    ...
    if args.cuda:
        if torch.cuda.device_count() > 1:
            ...
            teacher = load_my_state_dict(teacher,torch.load(teacher_path),False)#True
            ...
            teacher = load_my_state_dict(teacher,torch.load(teacher_path),False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="pvt_v2_b0")
    ...
    parser.add_argument("--norm-type", type=str, default='channel',choices=['channel','spatial','channel_mean'], help="kd normalize setting")
