...
# from .unifiedparsing.models.models import UPerNet
# import numpy as np
...
class PyramidVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=20, embed_dims=[64, 128, 256, 512],
                num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[3, 4, 6, 3],
                sr_ratios=[8, 4, 2, 1], num_stages=4, F4=False):
        super().__init__()
        ...
        # self.uper_head = UPerNet(nr_classes=16, fc_dim=512, use_softmax=False, fpn_dim=512)
        #PSP - PPM layers
        num_features = sum(embed_dims)+3
        self.cls = nn.Sequential(
            nn.Conv2d(num_features, 512, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=drop_path_rate),
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            patch_embed = getattr(self, f"patch_embed{i + 1}")#.to(device)
            pos_embed = getattr(self, f"pos_embed{i + 1}")#.to(device)
            pos_drop = getattr(self, f"pos_drop{i + 1}")#.to(device)
            block = getattr(self, f"block{i + 1}")#.to(device)
            x, (H, W) = patch_embed(x)
    ...
    def forward(self, x):
        ...
        for i in range(len(x)):
            out.append(F.interpolate(x[i],x_size[2:], mode='bilinear', align_corners=True))
        out = torch.cat(out,1)
        out = self.cls(out)
...
class pvt_tiny(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_tiny, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_classes=20,num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
